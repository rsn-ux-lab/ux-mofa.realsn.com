...
$(function () {
  ...
  */
  ...
  //   const $scrollEvtNode = document.querySelector('[data-section=전체이슈현황] [data-card=연관어비교] .keyword-list');
  //   $scrollEvtNode.classList.add('is-hide');

  //   let options = {
  //     root: null,
  //     // 감지를 할 조상노드 뷰포트(영역) - 기본 : 브라우저 viewport
  //     rootMargin: '0px',
  //     // "지정한 대상"의 위치를 기준으로,
  //     //  rootMargin 값만큼 더해진 위치에서 callback함수 실행
  //     //  ★ 단위값을 안쓰면 안됨!
  //     //  px - 기준위치에 절댓값을 더함
  //     //  % - 기준위치에 "지정한대상"의 노드 height 백분율을 기준으로 rootMargin 퍼센트값만큼 더함
  //     threshold: 0.7,
  //     // 지정한 대상의 위치를 퍼센트로 반환한 값(1 === 100%)을 받아와서 일치 할 경우 callback 함수 실행
  //     // 함수로 리턴값 불러와서 숫자 지정 가능함
  //     // root를 기준으로 대상요소가 얼마만큼 보일때 콜백함수를 실행할지 0~1 사이의 값으로 나타낸다.
  //     // 만약 요소가 25%씩 보일때마다 실행하고 싶다면 [0, 0.25, 0.5, 0.75, 1] 와 같이 배열로 작성하면 된다. (default: 1)
  //   };

  //   let moveTheTreemap = (obs, ins) => {
  //     if (obs[0].isIntersecting) $scrollEvtNode.classList.add('is-show');
  //     else $scrollEvtNode.classList.remove('is-show');
  //   };
  //   let observer = new IntersectionObserver(moveTheTreemap, options);

  //   observer.observe($scrollEvtNode);
  // }
  /*
  ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
  */
  {
    ...
    for (let i = 0; i < $scrollEvtNode.length; i++) {
      ...
      //   // 감지를 할 조상노드 뷰포트(영역) - 기본 : 브라우저 viewport
      //   rootMargin: '0px',
      //   //  roow(viewPort)을 rootMargin 값 만큼 늘이거나 축소시킴
      //   //   예) viewPort크기 1000px일때
      //   //       rootMargin: '-25%' 지정시
      //   //       observer가 감지하는 viewport의 크기는 500px이 됨
      //   //  ★ 단위값을 "꼭" 써줘야 함!
      //   //  px - 기준위치에 절댓값
      //   //  % - 기준위치에 "지정한대상"의 노드 height 백분율을 기준으로 rootMargin 퍼센트값만큼 더함
      //   threshold: 1,
