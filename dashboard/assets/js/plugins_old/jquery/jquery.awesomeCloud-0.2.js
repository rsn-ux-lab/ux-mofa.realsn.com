...
(function ($) {
	...
	var pluginName = "awesomeCloud", // name of the plugin, mainly for canvas IDs
		defaultSettings = {
			...
			"options": {
				"color": "random-light", // if "random-light" or "random-dark", color.start and color.end are ignored
	...
	AwesomeCloud.prototype = {
		create: function () {
			...
			this.container.children().each(function (i, e) {
				...
				var textColor = ( this.style.color ) ? this.style.color : null;

				$this.words.push([ tag , currentWeight, link, target, textColor]);
			});
		...
		putWord: function (word, weight, link, target, txtColor) {
			var $this = this,
				...
				if (points.shuffle().some(function (gxy) {
						if ($this.canFitText(gxy[ 0 ], gxy[ 1 ], gw, gh)) {
							...
							if (rotate) {
								...
								//console.log( hexToRgb( txtColor ) );
								
								if( txtColor != null ) fctx.fillStyle = rgbToHex( txtColor );
								else fctx.fillStyle = $this.wordcolor(word, weight, fontSize, R - r, gxy[ 2 ]);
								...
								$this.ctx.font = font;
								if( txtColor != null ) $this.ctx.fillStyle = rgbToHex( txtColor );
								else $this.ctx.fillStyle = $this.wordcolor(word, weight, fontSize, R - r, gxy[ 2 ]);
