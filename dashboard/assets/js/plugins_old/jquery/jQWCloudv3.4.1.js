(function( $ ){
    ...
    Word.prototype = {
        ...
        _setSpan_Size: function(){
            ...
            $(span).html( "<span class='bg' style='border-color:" + color + ";background:" + color + "'></span><span>" + this.word + "</span>" );
    ...
    WordCloud.prototype = {
        _init: function(options){
            ...
            this._start();
            this.options.afterCloudRender( wordObjs );
            this._final();
        },
        ...
        _draw: function() {
            ...
            this.words.filter( function( $item, index ){
                ...
                if(_this.options.cloud_color!=null)
                    wordConfigObj['color']=_this.options.cloud_color;
                else
                    wordConfigObj['color']=currWord.color;
                
                if(_this.options.padding_left!=null)
                    wordConfigObj['padding_left']=_this.options.padding_left;
                
                wordConfigObj['word_class']=currWord.word_class;
                
                if(_this.options.cloud_font_family!=null)
                    wordConfigObj['font_family']=_this.options.cloud_font_family;
                else	
                ...
                if(_this.options.word_common_classes!=null)
                    $(wordObj.span).addClass(_this.options.word_common_classes);
                
                var data = JSON.parse( JSON.stringify( _this.words[ index ] ) );
                $(wordObj.span).on("click",function( $e ){
                    _this.options.word_click( $e, currWord );
                });
                ...
                if(index==0)
                    _this._placeFirstWord(wordObj);
                else	
                    _this._placeOtherWord(wordObj);		
            })
        },
